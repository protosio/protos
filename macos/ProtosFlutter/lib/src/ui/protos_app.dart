                    ...
                    child: SizedBox.square(
                      ...
                      child: IconButton(
                        ...
                        onPressed: model.isBusy
                            ? null
                            : () => unawaited(model.run(model.refreshAll)),
          ...
          CommandBar(
            children: [
              CommandButton(
                ...
                action: (model) => model.api.startApp(selectedName),
              ),
              CommandButton(
                label: 'Stop',
                icon: Icons.stop,
                enabled: selectedName.nonEmpty != null,
                action: (model) => model.api.stopApp(selectedName),
              ),
              CommandButton(
                label: 'Remove',
                ...
                destructive: true,
                enabled: selectedName.nonEmpty != null,
                action: (model) => model.api.removeApp(selectedName),
              ),
              CommandButton(
                label: 'Logs',
          ...
          CommandBar(
            children: [
              CommandButton(
                ...
                destructive: true,
                enabled: selectedName.nonEmpty != null,
                action: (model) => model.api.removeProvisioner(selectedName),
              ),
            ],
          ),
        ],
...
class _InstancesViewState extends State<InstancesView> {
          ...
          if (deployProvisioner.isEmpty)
            ...
            _DeploymentForm(
              ...
              onChangeProvisioner: () {
                ...
                unawaited(model.run(model.refreshInstanceDeployOptions));
              },
  ...
  void _selectProvisioner(AppModel model, pb.InstanceDeployFieldOption option) {
    ...
    unawaited(
      model.run(() async {
        await model.refreshInstanceDeployOptions(provisioner: option.value);
                ...
                onPressed: provisioner.text.nonEmpty == null
                    ...
                    : () => unawaited(
                        model.run(() async {
                          ...
                          model.setProvisionerImages(response.images);
                        }),
                      ),
...
class SqlResultPanel extends StatelessWidget {
